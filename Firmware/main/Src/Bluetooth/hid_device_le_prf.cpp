...
static const uint8_t hidReportMap[] = {
    ...
    0x19,
    0x01, //     Usage Minimum (01) - Button 1
    0x29,
    0x03, //     Usage Maximum (03) - Button 3
    0x15,
    ...
    0x03, //     Report Count (3)
    0x81,
    0x02, //     Input (Data, Variable, Absolute) - Button states
    0x75,
    0x05, //     Report Size (5)
    0x95,
    0x01, //     Report Count (1)
    0x81,
    0x01, //     Input (Constant) - Padding or Reserved bits
    0x05,
    0x01, //     Usage Page (Generic Desktop)
    0x09,
    0x30, //     Usage (X)
    ...
    0x03, //     Report Count (3)
    0x81,
    0x06, //     Input (Data, Variable, Relative) - X & Y coordinate
    0xC0, //   End Collection
    0xC0, // End Collection
    ...
    //
    0x05,
    0x0C, // Usage Pg (Consumer Devices)
    0x09,
    ...
    0x05,
    0x09, //     Usage Pg (Button)
    0x19,
    0x01, //     Usage Min (Button 1)
    0x29,
    0x0A, //     Usage Max (Button 10)
    0x15,
    0x01, //     Logical Min (1)
    0x25,
    0x0A, //     Logical Max (10)
    ...
    0xB4, //   Usage (Rewind)
    0x09,
    0xB5, //   Usage (Scan Next)
    0x09,
    0xB6, //   Usage (Scan Prev)
    0x09,
    0xB7, //   Usage (Stop)
    0x15,
    0x01, //   Logical Min (1)
    ...
    0x09,
    0xA5, // Usage(Vendor Defined)
    0xA1,
    0x01, // Collection(Application)
    0x85,
    0x04, // Report Id (4)
    0x09,
    0xA6, // Usage(Vendor defined)
    0x09,
    0xA9, // Usage(Vendor defined)
    0x75,
    0x08, // Report Size
    0x95,
    0x7F, // Report Count = 127 Btyes
    0x91,
    0x02, // Output(Data, Variable, Absolute)
    0xC0, // End Collection
#endif
...
static const esp_gatts_attr_db_t bas_att_db[BAS_IDX_NB] = {
    ...
    [BAS_IDX_BATT_LVL_PRES_FMT] = {{ESP_GATT_AUTO_RSP},
                                   {ESP_UUID_LEN_16,
                                    ...
                                    ESP_GATT_PERM_READ,
                                    sizeof(struct prf_char_pres_fmt),
                                    0,
                                    NULL}},
};
...
static esp_gatts_attr_db_t hidd_le_gatt_db[HIDD_LE_IDX_NB] = {
    ...
    [HIDD_LE_IDX_REPORT_MAP_EXT_REP_REF] =
        {{ESP_GATT_AUTO_RSP},
         {ESP_UUID_LEN_16,
          (uint8_t*)&hid_repot_map_ext_desc_uuid,
          ESP_GATT_PERM_READ,
          sizeof(uint16_t),
          sizeof(uint16_t),
    ...
    [HIDD_LE_IDX_REPORT_MOUSE_REP_REF] = {{ESP_GATT_AUTO_RSP},
                                          {ESP_UUID_LEN_16,
                                           (uint8_t*)&hid_report_ref_descr_uuid,
                                           ESP_GATT_PERM_READ,
                                           sizeof(hidReportRefMouseIn),
                                           sizeof(hidReportRefMouseIn),
                                           hidReportRefMouseIn}},
    ...
    [HIDD_LE_IDX_BOOT_KB_IN_REPORT_VAL] = {{ESP_GATT_AUTO_RSP},
                                           {ESP_UUID_LEN_16,
                                            (uint8_t*)&hid_kb_input_uuid,
                                            ESP_GATT_PERM_READ,
                                            HIDD_LE_BOOT_REPORT_MAX_LEN,
                                            0,
                                            NULL}},
...
void esp_hidd_prf_cb_hdl(esp_gatts_cb_event_t event,
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT: {
            ...
            hidd_param.init_finish.state =
                static_cast<esp_hidd_init_state_t>(param->reg.status);
            if (param->reg.app_id == HIDD_APP_ID) {
                hidd_le_env.gatt_if = gatts_if;
                if (hidd_le_env.hidd_cb != NULL) {
                    (hidd_le_env.hidd_cb)(ESP_HIDD_EVENT_REG_FINISH,
                                          &hidd_param);
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            if (param->add_attr_tab.num_handle == BAS_IDX_NB &&
                ...
                ESP_LOGI(HID_LE_PRF_TAG,
                         "%s(), start added the hid service to the stack "
                         "database. incl_handle = %d",
                         __func__,
                         incl_svc.start_hdl);
...
void hidd_le_create_service(esp_gatt_if_t gatts_if) {
    /* Here should added the battery service first, because the hid service
       should include the battery service. After finish to added the battery
       service then can added the hid service. */
        ...
        for (idx = 0; idx < PROFILE_NUM; idx++) {
            if (gatts_if ==
                ...
                if (heart_rate_profile_tab[idx].gatts_cb) {
                    heart_rate_profile_tab[idx].gatts_cb(event,
                                                         gatts_if,
                                                         param);
                }
            }
